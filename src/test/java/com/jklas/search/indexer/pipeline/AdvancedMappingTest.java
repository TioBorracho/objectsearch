...
public class AdvancedMappingTest {
	...
	@SuppressWarnings("unused")
	@Indexable(makeSubclassesIndexable = true, indexName="SUPER") @TextProcessor(OneTermTextProcessor.class)
	...
	@SuppressWarnings("unused")
	@LangId(value="CHILD_LANG") @TextProcessor(NullProcessor.class)
	private class ChildWithOverrides extends SuperWithCascade {
		@SearchId public String childId;
		@IndexSelector public String anIndexId = "CHILD";
	}
	...
	@TextProcessor(OneTermTextProcessor.class) @LangId("LANG_SON")
	private class Son extends Father {
		@LangSelector public final static String lang= "SON_LANG";
	...
	public void CascadesOverridesOnMultipleLevels() throws SearchEngineMappingException, SearchEngineException, IllegalArgumentException, IllegalAccessException {
		...
		Field langField = mapping.getLanguageSelectorField(); langField.setAccessible(true);
	...
	public class DummyContainter {
		@SearchCollection(reference=IndexReference.BOTH) List<Object> objectList1;
		@SearchCollection(reference=IndexReference.SELF) List<Object> objectList2;
	}
	...
	public void SearchCollectionsAreMapped() throws SearchEngineMappingException, SearchEngineException, IllegalArgumentException, IllegalAccessException, SecurityException, NoSuchFieldException {
		...
		for (Entry<Field, MappedFieldDescriptor> entries : mappedFields.entrySet()) {
			...
			if(entries.getKey().equals(DummyContainter.class.getDeclaredField("objectList1")))
				Assert.assertEquals(IndexReference.BOTH, entries.getValue().getReferenceType());
			else
				Assert.assertEquals(IndexReference.SELF, entries.getValue().getReferenceType());			
		}
	}
	...
	public class DummyContainterAndIndexable {
		@SearchCollection(reference=IndexReference.BOTH) List<Object> objectList;
		@SearchField String a;
		@SearchId int id;
	}
	...
	public class DummyWithIndexableByMapping {		
		@SearchId int id = 0;
		@SearchContained SingleAttributeEntity contained = new SingleAttributeEntity(1,"single");
	}
